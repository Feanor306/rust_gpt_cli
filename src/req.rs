...
pub async fn query_completions(client: &reqwest::Client, rp: &RequestParams, api_key: &String) {
    ...
    while let Some(item) = stream.next().await {
        ...
        let clean_json = helpers::sanitize_json(&json_string, &rp.model, false);

        for cj in clean_json {
            let msg: String = helpers::extract_msg_from_json(&cj, false);
            if msg.len() == 0 {
                continue;
            }
...
pub async fn query_chat_completions(client: &reqwest::Client, rp: &mut RequestParams, api_key: &String) {
    crate::log::log_info(&format!("{}{}", helpers::entity_line(true, &rp.model, &"PROMPT".into()), &rp.prompt));
    ...
    let mut gr = GPTResponse::new(&rp.messages.get_last_message());
    let mut stream = res.unwrap().bytes_stream();
    println!("{}", helpers::entity_line(false, &rp.model, &"GPT".into()));
    while let Some(item) = stream.next().await {
        let bs = item.unwrap();
        let json_string = String::from_utf8(bs.to_vec()).unwrap();
        let clean_json = helpers::sanitize_json(&json_string, &rp.model, true);

        for cj in clean_json {
            let msg: String = helpers::extract_msg_from_json(&cj, true);
            if msg.len() == 0 {
                continue;
            }
    ...
    log::log_info(&format!("{}{}", helpers::entity_line(true, &rp.model, &"GPT".into()), gr.full_response));
    log::log_info(&format!("{}", helpers::entity_line(true, &rp.model, &"DONE".into())));
    rp.messages.add_assistant_msg(&gr.full_response);
}
